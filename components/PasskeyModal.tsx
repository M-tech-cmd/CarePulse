...
const PasskeyModal = () => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined" && !isAdminRoute) {
      ...
      if (isAdminParam && accessKey !== adminPasskey) {
        setOpen(true);
      } else if (accessKey === adminPasskey) {
        setOpen(false);
        if (isAdminParam) router.push("/admin");
      }
    }
  ...
  const validatePasskey = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    if (passkey === adminPasskey) {
      ...
      setOpen(false);
      router.push("/admin"); // Ensure redirect after validation
    } else {
